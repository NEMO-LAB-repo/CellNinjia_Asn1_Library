...
#include "NR5G-L2-UL-TB-S-H.h"
#include "NR5G-L2-UL-TB-TTI-H.h"
#include "NR5G-L2-UL-TB-Content.h"
#include "host.h"

void convert_S_H_B872(uint8_t *hex_data, int *index)
{
    *index+=8;
}

void convert_TTI_H_B872(uint8_t *hex_data, int *index)
{
    *index+=2;
...
void convert_TB_B872(uint8_t *hex_data, int *index)
{
    convert_endianess(hex_data, index, 2);
    convert_endianess(hex_data, index, 2);
...
{
    NR5G_L2_UL_TB_S_H_t *t_S_H = 0;
    asn_dec_rval_t rval_S_H;
    NR5G_L2_UL_TB_TTI_H_t *t_TTI_H = 0;
    asn_dec_rval_t rval_TTI_H;
    NR5G_L2_UL_TB_Content_t *t_Content = 0;
    asn_dec_rval_t rval_Content;

    /*S_H*/
    int start_S_H = *index;
    convert_S_H_B872(hex_data, index);
    int S_H_length = *index - start_S_H;

    rval_S_H = uper_decode(0, &asn_DEF_NR5G_L2_UL_TB_S_H, (void **)&t_S_H, hex_data + start_S_H, S_H_length, 0, 0);
    if (rval_S_H.code != RC_OK)
    {
        printf("rval_S_H decode error\n");
        return;
    }
    xer_fprint(stdout, &asn_DEF_NR5G_L2_UL_TB_S_H, t_S_H);
    ...
    {
        int start_TTI = *index;
        convert_TTI_H_B872(hex_data, index);
        int TTI_length = *index - start_TTI;

        rval_TTI_H = uper_decode(0, &asn_DEF_NR5G_L2_UL_TB_TTI_H, (void **)&t_TTI_H, hex_data + start_TTI, TTI_length, 0, 0);
        if (rval_TTI_H.code != RC_OK)
        {
            printf("rval_TTI decode error\n");
            return;
        }
        xer_fprint(stdout, &asn_DEF_NR5G_L2_UL_TB_TTI_H, t_TTI_H);

        int num_tb = hex_data[start_TTI + 4];

        for(int k=0;k<num_tb;k++)
        {
            int start_TB = *index;
            convert_TB_B872(hex_data, index);
            int TB_length = *index - start_TB;

            rval_Content = uper_decode(0, &asn_DEF_NR5G_L2_UL_TB_Content, (void **)&t_Content, hex_data + start_TB, TB_length, 0, 0);
            if (rval_Content.code != RC_OK)
            {
                printf("rval_Content decode error\n");
                return;
            }
            xer_fprint(stdout, &asn_DEF_NR5G_L2_UL_TB_Content, t_Content);

            int mce_length = (hex_data[start_TB + 16] & 0x3F);
