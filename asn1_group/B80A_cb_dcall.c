...
void decode_NAS_80A_v1(uint8_t *hex_data, size_t length, int *index, int msg_len, uint64_t time_in_us_total)
{
    ...
    *index+=7;
    ...
    FILE *pcap_file = fopen("B80A.pcap", "rb");
    if (pcap_file == NULL) 
    {
        printf("Creating new pcap file: B80A.pcap\n");
        pcap_file = fopen("B80A.pcap", "wb");
        if (pcap_file == NULL) {
    ...
    fclose(pcap_file);
    printf("Pcap file B80A.pcap updated successfully\n");
}
